// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/auth/change-password": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cambiar la contraseña del usuario autenticado",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Autenticación"
                ],
                "summary": "Cambiar contraseña de usuario",
                "parameters": [
                    {
                        "description": "Solicitud de cambio de contraseña",
                        "name": "password",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.ChangePasswordRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Contraseña cambiada exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.ChangePasswordResponse"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/auth/login": {
            "post": {
                "description": "Autenticar usuario con nombre de usuario y contraseña",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Autenticación"
                ],
                "summary": "Inicio de sesión de usuario",
                "parameters": [
                    {
                        "description": "Credenciales de inicio de sesión",
                        "name": "credentials",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.LoginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Inicio de sesión exitoso",
                        "schema": {
                            "$ref": "#/definitions/models.LoginResponse"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "Credenciales inválidas",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/auth/logout": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cerrar la sesión del usuario actual",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Autenticación"
                ],
                "summary": "Cerrar sesión",
                "responses": {
                    "200": {
                        "description": "Cierre de sesión exitoso",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/messages": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Envía un mensaje del usuario autenticado al otro usuario del sistema.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Mensajes"
                ],
                "summary": "Enviar un mensaje",
                "parameters": [
                    {
                        "description": "Carga útil del mensaje",
                        "name": "message",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/controllers.sendMessageRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Mensaje enviado exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.Message"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/messages/conversation": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Recupera el historial de mensajes entre el usuario autenticado y el otro usuario del sistema, con paginación.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Mensajes"
                ],
                "summary": "Obtener conversación",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de página (por defecto 1)",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Mensajes por página (por defecto 10)",
                        "name": "per_page",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Historial de conversación",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/models.Message"
                            }
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/messages/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Edita un mensaje enviado por el usuario autenticado, si es dentro de 1 hora de enviado.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Mensajes"
                ],
                "summary": "Editar un mensaje",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID del mensaje",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Nuevo contenido del mensaje",
                        "name": "message",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/controllers.editMessageRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Mensaje actualizado exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.Message"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "No encontrado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Elimina un mensaje para el remitente y el destinatario.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Mensajes"
                ],
                "summary": "Eliminar un mensaje",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID del mensaje",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Sin Contenido"
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "No encontrado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/messages/{id}/delivered": {
            "patch": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Marca un mensaje como entregado al destinatario.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Mensajes"
                ],
                "summary": "Marcar mensaje como entregado",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID del mensaje",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Mensaje marcado como entregado",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "No encontrado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/messages/{id}/read": {
            "patch": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Marca un mensaje como leído por el destinatario.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Mensajes"
                ],
                "summary": "Marcar mensaje como leído",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID del mensaje",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Mensaje marcado como leído",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "No encontrado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/todos": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Obtener una lista de tareas con filtros opcionales por estado, creador y ordenamiento por fecha de creación.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Tareas"
                ],
                "summary": "Listar tareas",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Filtrar por ID de usuario creador.",
                        "name": "creator_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "all",
                            "completed",
                            "incompleted",
                            "completed_by_me"
                        ],
                        "type": "string",
                        "default": "all",
                        "description": "Filtrar por estado.",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Buscar por título o descripción.",
                        "name": "search",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "asc",
                            "desc"
                        ],
                        "type": "string",
                        "default": "desc",
                        "description": "Orden por fecha de creación.",
                        "name": "sort_order",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 1,
                        "description": "Número de página",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Elementos por página",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Tareas recuperadas exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.TodoListResponse"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Crear una nueva tarea (solo para usuarios autorizados: anyel, alexis)",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Tareas"
                ],
                "summary": "Crear una nueva tarea",
                "parameters": [
                    {
                        "description": "Solicitud de creación de tarea",
                        "name": "todo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.CreateTodoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Tarea creada exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.CreateTodoResponse"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/todos/{id}": {
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Actualizar título y descripción de la tarea (solo el creador puede actualizar sus tareas)",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Tareas"
                ],
                "summary": "Actualizar detalles de la tarea",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID de la tarea",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Solicitud de actualización de tarea",
                        "name": "todo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.UpdateTodoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Tarea actualizada exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.Todo"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Tarea no encontrada",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Eliminar una tarea (solo el creador puede eliminar sus tareas)",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Tareas"
                ],
                "summary": "Eliminar una tarea",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID de la tarea",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Tarea eliminada exitosamente",
                        "schema": {
                            "$ref": "#/definitions/models.SuccessResponse"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Tarea no encontrada",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            },
            "patch": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Actualizar el estado de la tarea (completado) para el usuario autenticado",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Tareas"
                ],
                "summary": "Actualizar estado de la tarea",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID de la tarea",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Solicitud de actualización de estado de tarea",
                        "name": "status",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.UpdateTodoStatusRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Estado de la tarea actualizado",
                        "schema": {
                            "$ref": "#/definitions/models.CompleteTodoResponse"
                        }
                    },
                    "400": {
                        "description": "Solicitud incorrecta",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "403": {
                        "description": "Prohibido",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Tarea no encontrada",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Error interno del servidor",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/ws": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Actualiza la conexión HTTP a una conexión WebSocket para comunicación en tiempo real.",
                "tags": [
                    "Mensajes"
                ],
                "summary": "Conexión WebSocket",
                "responses": {
                    "101": {
                        "description": "Cambiando Protocolos"
                    },
                    "401": {
                        "description": "No autorizado",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "controllers.editMessageRequest": {
            "type": "object",
            "properties": {
                "content": {
                    "description": "Nuevo contenido del mensaje",
                    "type": "string"
                }
            }
        },
        "controllers.sendMessageRequest": {
            "type": "object",
            "properties": {
                "content": {
                    "description": "Contenido del mensaje a enviar",
                    "type": "string"
                }
            }
        },
        "models.ChangePasswordRequest": {
            "description": "Solicitud de cambio de contraseña",
            "type": "object",
            "required": [
                "new_password"
            ],
            "properties": {
                "new_password": {
                    "description": "Nueva contraseña en texto plano (mínimo 6 caracteres)",
                    "type": "string",
                    "example": "newpassword"
                }
            }
        },
        "models.ChangePasswordResponse": {
            "description": "Respuesta de cambio de contraseña",
            "type": "object",
            "properties": {
                "message": {
                    "description": "Mensaje de confirmación del cambio",
                    "type": "string",
                    "example": "¡Contraseña actualizada con éxito! ✨"
                }
            }
        },
        "models.CompleteTodoResponse": {
            "description": "Respuesta de finalización de tarea",
            "type": "object",
            "properties": {
                "message": {
                    "description": "Mensaje de resultado",
                    "type": "string",
                    "example": "¡Bien hecho! Tarea marcada como completada. 🎉"
                },
                "todo": {
                    "description": "Estado actualizado de la tarea",
                    "allOf": [
                        {
                            "$ref": "#/definitions/models.Todo"
                        }
                    ]
                }
            }
        },
        "models.CreateTodoRequest": {
            "description": "Solicitud de creación de tarea",
            "type": "object",
            "required": [
                "title"
            ],
            "properties": {
                "description": {
                    "description": "Descripción opcional de la tarea",
                    "type": "string",
                    "example": "Ir al supermercado y comprar frutas y verduras"
                },
                "title": {
                    "description": "Título de la tarea (obligatorio)",
                    "type": "string",
                    "example": "Comprar comida"
                }
            }
        },
        "models.CreateTodoResponse": {
            "description": "Respuesta de creación de tarea",
            "type": "object",
            "properties": {
                "message": {
                    "description": "Mensaje de confirmación",
                    "type": "string",
                    "example": "¡Tarea creada con éxito! 🚀"
                },
                "todo": {
                    "description": "Datos completos de la tarea recién creada",
                    "allOf": [
                        {
                            "$ref": "#/definitions/models.Todo"
                        }
                    ]
                }
            }
        },
        "models.ErrorResponse": {
            "description": "Respuesta de error",
            "type": "object",
            "properties": {
                "code": {
                    "description": "Código de estado HTTP asociado al error",
                    "type": "integer",
                    "example": 400
                },
                "error": {
                    "description": "Tipo o categoría del error",
                    "type": "string",
                    "example": "Mensaje de error"
                },
                "message": {
                    "description": "Descripción detallada del error para el usuario",
                    "type": "string",
                    "example": "Error detallado"
                }
            }
        },
        "models.LoginRequest": {
            "description": "Credenciales de inicio de sesión",
            "type": "object",
            "required": [
                "password",
                "username"
            ],
            "properties": {
                "password": {
                    "description": "Contraseña en texto plano (obligatorio)",
                    "type": "string",
                    "example": "anyel"
                },
                "username": {
                    "description": "Nombre de usuario (obligatorio)",
                    "type": "string",
                    "example": "anyel"
                }
            }
        },
        "models.LoginResponse": {
            "description": "Respuesta de inicio de sesión con token e información de usuario",
            "type": "object",
            "properties": {
                "message": {
                    "description": "Mensaje de confirmación",
                    "type": "string",
                    "example": "Inicio de sesión exitoso"
                },
                "token": {
                    "description": "Token JWT para autenticación de solicitudes",
                    "type": "string",
                    "example": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
                },
                "unread_count": {
                    "description": "Cantidad de mensajes no leídos para pintar el badge inicial",
                    "type": "integer",
                    "example": 3
                },
                "user": {
                    "description": "Datos del usuario autenticado (sin contraseña)",
                    "allOf": [
                        {
                            "$ref": "#/definitions/models.User"
                        }
                    ]
                }
            }
        },
        "models.Message": {
            "type": "object",
            "properties": {
                "content": {
                    "description": "Contenido textual del mensaje",
                    "type": "string"
                },
                "created_at": {
                    "description": "Fecha y hora de creación del mensaje",
                    "type": "string"
                },
                "id": {
                    "description": "Identificador único del mensaje",
                    "type": "integer"
                },
                "receiver": {
                    "description": "Información resumida del destinatario",
                    "allOf": [
                        {
                            "$ref": "#/definitions/models.UserSummary"
                        }
                    ]
                },
                "receiver_id": {
                    "description": "ID del usuario que recibe el mensaje",
                    "type": "integer"
                },
                "sender": {
                    "description": "Información resumida del remitente",
                    "allOf": [
                        {
                            "$ref": "#/definitions/models.UserSummary"
                        }
                    ]
                },
                "sender_id": {
                    "description": "ID del usuario que envió el mensaje",
                    "type": "integer"
                },
                "status": {
                    "description": "Estado del mensaje: \"sent\", \"delivered\" o \"read\"",
                    "type": "string"
                },
                "updated_at": {
                    "description": "Fecha y hora de la última actualización",
                    "type": "string"
                }
            }
        },
        "models.SuccessResponse": {
            "description": "Respuesta de éxito",
            "type": "object",
            "properties": {
                "data": {
                    "description": "Datos opcionales retornados junto al mensaje"
                },
                "message": {
                    "description": "Mensaje descriptivo del resultado exitoso",
                    "type": "string",
                    "example": "Operación exitosa"
                }
            }
        },
        "models.Todo": {
            "description": "Información del elemento de tarea",
            "type": "object",
            "properties": {
                "completed_alexis": {
                    "description": "Indica si Alexis marcó la tarea como completada",
                    "type": "boolean",
                    "example": false
                },
                "completed_anyel": {
                    "description": "Indica si Anyel marcó la tarea como completada",
                    "type": "boolean",
                    "example": true
                },
                "created_at": {
                    "description": "Fecha y hora de creación",
                    "type": "string",
                    "example": "2024-01-01T00:00:00Z"
                },
                "creator_id": {
                    "description": "ID del usuario que creó la tarea",
                    "type": "integer",
                    "example": 1
                },
                "creator_username": {
                    "description": "Nombre de usuario del creador",
                    "type": "string",
                    "example": "anyel"
                },
                "description": {
                    "description": "Descripción detallada de la tarea",
                    "type": "string",
                    "example": "Ir al supermercado y comprar..."
                },
                "id": {
                    "description": "Identificador único de la tarea",
                    "type": "integer",
                    "example": 1
                },
                "is_completed": {
                    "description": "true solo cuando ambos usuarios han completado la tarea",
                    "type": "boolean",
                    "example": false
                },
                "title": {
                    "description": "Título descriptivo de la tarea",
                    "type": "string",
                    "example": "Comprar comida"
                },
                "updated_at": {
                    "description": "Fecha y hora de la última actualización",
                    "type": "string",
                    "example": "2024-01-01T00:00:00Z"
                }
            }
        },
        "models.TodoListResponse": {
            "description": "Respuesta de lista de tareas",
            "type": "object",
            "properties": {
                "last_page": {
                    "description": "Última página disponible",
                    "type": "integer",
                    "example": 1
                },
                "message": {
                    "description": "Mensaje informativo",
                    "type": "string",
                    "example": "Aquí están todas las tareas. ¡Vamos a completarlas! 💪"
                },
                "page": {
                    "description": "Página actual",
                    "type": "integer",
                    "example": 1
                },
                "per_page": {
                    "description": "Cantidad de tareas por página",
                    "type": "integer",
                    "example": 10
                },
                "todos": {
                    "description": "Lista de tareas encontradas",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/models.Todo"
                    }
                },
                "total": {
                    "description": "Total de tareas que coinciden con el filtro",
                    "type": "integer",
                    "example": 5
                }
            }
        },
        "models.UpdateTodoRequest": {
            "description": "Solicitud de actualización de tarea",
            "type": "object",
            "required": [
                "title"
            ],
            "properties": {
                "description": {
                    "description": "Nueva descripción de la tarea",
                    "type": "string",
                    "example": "Ir al supermercado y comprar frutas y verduras"
                },
                "title": {
                    "description": "Nuevo título de la tarea (obligatorio)",
                    "type": "string",
                    "example": "Comprar comida"
                }
            }
        },
        "models.UpdateTodoStatusRequest": {
            "description": "Solicitud de actualización de estado de tarea",
            "type": "object",
            "properties": {
                "completed": {
                    "description": "true para marcar como completada, false para desmarcar",
                    "type": "boolean",
                    "example": true
                }
            }
        },
        "models.User": {
            "description": "Información de cuenta de usuario",
            "type": "object",
            "properties": {
                "created_at": {
                    "description": "Fecha de creación de la cuenta",
                    "type": "string",
                    "example": "2024-01-01T00:00:00Z"
                },
                "id": {
                    "description": "Identificador único del usuario",
                    "type": "integer",
                    "example": 1
                },
                "name": {
                    "description": "Nombre real del usuario",
                    "type": "string",
                    "example": "Anyel"
                },
                "updated_at": {
                    "description": "Fecha de la última actualización",
                    "type": "string",
                    "example": "2024-01-01T00:00:00Z"
                },
                "username": {
                    "description": "Nombre de usuario único para autenticación",
                    "type": "string",
                    "example": "anyel"
                }
            }
        },
        "models.UserSummary": {
            "description": "Información resumida de usuario",
            "type": "object",
            "properties": {
                "id": {
                    "description": "Identificador único del usuario",
                    "type": "integer",
                    "example": 1
                },
                "name": {
                    "description": "Nombre real del usuario",
                    "type": "string",
                    "example": "Anyel"
                },
                "username": {
                    "description": "Nombre de usuario",
                    "type": "string",
                    "example": "anyel"
                }
            }
        }
    },
    "securityDefinitions": {
        "BearerAuth": {
            "description": "Escriba \"Bearer\" seguido de un espacio y el token JWT.",
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "localhost:4418",
	BasePath:         "/",
	Schemes:          []string{},
	Title:            "API Backend de LoveApp",
	Description:      "Esta es una API de gestión de tareas para parejas. Permite a dos usuarios (anyel y alexis) crear, gestionar y completar tareas juntos.",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
